...
public class JavaBlitter implements Blitter
{
	...
	static
	{
		...
		for (int i = 1; i < 256; i++)
		{
			for (int j = 0; j < i; j++)
			{
				divLUT[i][j] = (byte) (((float) j / 255) / ((float) i / 255)
						* 255 + 0.5f);
			}
			for (int j = i; j < 256; j++)
			{
...
//						h, sysxform, comp, bgcolor, clip);
			...
			Color bgcolor, MultiRectArea clip)
	{
		...
		javaBlt(srcX, srcY, srcSurf.getWidth(), srcSurf.getHeight(),
				...
				dstSurf.getColorModel(), dstSurf.getRaster(), width, height,
				comp, bgcolor, clip);
	}

	public void javaBlt(int srcX, int srcY, int srcW, int srcH,
			ColorModel srcCM, Raster srcRast, int dstX, int dstY, int dstW,
			int dstH, ColorModel dstCM, WritableRaster dstRast, int width,
			int height, Composite comp, Color bgcolor, MultiRectArea clip)
	{
		...
		for (int i = 1; i < clipRects[0]; i += 4)
		{
			...
			if (isAlphaComp)
			{
				alphaCompose(_sx, _sy, srcCM, srcRast, _dx, _dy, dstCM,
						dstRast, _w, _h, rule, alpha, bgcolor);
			}
			else if (isXORComp)
			{
				xorCompose(_sx, _sy, srcCM, srcRast, _dx, _dy, dstCM, dstRast,
						_w, _h, xorcolor);
			}
		}
	}
			...
			int width, int height, int rule, float alpha, Color bgcolor)
	{
		...
		for (int sy = srcY, dy = dstY, srcYMax = srcY + height; sy < srcYMax; sy++, dy++)
		{
			...
			{
				srcPixel = srcRast.getDataElements(sx, sy);
				srcRGB = srcPixel;
				if (bgcolor == null)
				{
					dstPixel = dstRast.getDataElements(dx, dy);
					dstRGB = dstPixel;
				}

				dstRGB = compose(srcRGB, dstRGB, dstCM.hasAlpha(), rule,
						srcConstAllpha);
	...
	private void transformedBlit(ColorModel srcCM, Raster srcR, int srcX,
			...
			Composite comp, Color bgcolor, MultiRectArea clip)
	{
		...
		for (int i = 1; i < clipRects[0]; i += 4)
		{
			...
			int sx = (int) ((m[4] + m[0] * (bounds.x - translateX) + m[2]
					* (bounds.y - translateY)) * 256 + 0.5);
			int sy = (int) ((m[5] + m[1] * (bounds.x - translateX) + m[3]
					* (bounds.y - translateY)) * 256 + 0.5);
							...
							&& py < maxSrcY)
					{
						switch (compType)
						{
							case AlphaCompositeMode:
								srcVal = srcR.getDataElements(px, py);
								srcRGB = srcVal;
								if (bgcolor != null)
								{
									dstRGB = bgRGB;
								...
								{
									dstVal = dstR.getDataElements(x, y);
									dstRGB = dstVal;
								}
								dstRGB = compose(srcRGB, dstRGB,
										dstCM.hasAlpha(), rule, srcConstAlpha);
								dstVal = dstRGB;
							...
							default:
								...
								throw new IllegalArgumentException(
										Messages.getString("awt.37", //$NON-NLS-1$
												comp.getClass()));
						}
					}
					sx += hx;
	...
	public static Rectangle2D getBounds2D(AffineTransform at, Rectangle r)
	{
		...
		float[] corners = { x, y, x + width + 1, y, x + width + 1,
				y + height + 1, x, y + height + 1 };
	...
	private int compose(int srcRGB, int dstRGB, boolean dstHasAlpha, int rule,
			int srcConstAlpha)
	{
