...
public class ConvolveOp implements BufferedImageOp, RasterOp
{
	...
	{
		if (src == null)
		{ // Should throw according to spec
	...
	private int slowFilter(Raster src, WritableRaster dst)
	{
		try
		{
			...
			// Processing bounds
			int[] pixels = null;
			pixels = src.getPixels(srcMinX, srcMinY, srcWidth, srcHeight,
					pixels);
			...
			for (int i = yOrigin; i < srcConvMaxY; i++)
			{
				for (int j = xOrigin; j < srcConvMaxX; j++)
				{
					...
					for (int hIdx = 0, rasterHIdx = i - yOrigin; hIdx < kHeight; hIdx++, rasterHIdx++)
					{
						...
						{
							int curIndex = rasterHIdx * rowLength + rasterWIdx
									* numBands;
		...
		{ // Something goes wrong, signal error
			return 1;
		}
		return 0;
	}
	...
	private int ippFilter(Raster src, WritableRaster dst, int imageType)
	{
		...
		return ippFilter32f(kernel.data, kernel.getWidth(), kernel.getHeight(),
				kernel.getXOrigin(), kernel.getYOrigin(), edgeCond, srcData,
				src.getWidth(), src.getHeight(), srcStride, dstData,
				dst.getWidth(), dst.getHeight(), dstStride, channels,
				skipChannel, offsets);
	}

	private native int ippFilter32f(float kernel[], int kWidth, int kHeight,
			int anchorX, int anchorY, int borderType, Object src, int srcWidth,
			int srcHeight, int srcStride, Object dst, int dstWidth,
			int dstHeight, int dstStride, int channels, boolean skipChannel,
			int offsets[]);
}
